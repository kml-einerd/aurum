...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Check for URL parameter and auto-load symbol
    const urlParams = new URLSearchParams(window.location.search);
    const symbolParam = urlParams.get('symbol');

    if (symbolParam) {
        const formattedSymbol = symbolParam.includes(':') ? symbolParam : `BMFBOVESPA:${symbolParam}`;
        document.getElementById('symbolInput').value = formattedSymbol;
        currentSymbol = formattedSymbol;
        performSymbolUpdate(formattedSymbol);
    }
});
